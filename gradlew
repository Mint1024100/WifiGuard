...
if [ ! -f "$GRADLE_USER_HOME/wrapper/dists" ]; then
...
if $cygwin || $darwin || $nonstop; then
    ...
    if [ $MAX_FD -eq "maximum" -o $MAX_FD -eq "max" ]; then
...
#   * Do NOT use $JAVA_OPTS, because the original command don't use it
#   * DO use $GRADLE_OPTS, because the original command uses it
set -- \
