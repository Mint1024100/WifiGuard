 ...
 * 
 * ИСПРАВЛЕНО:
 * - Используются валидные не подозрительные MAC-адреса (OUI Microsoft: 00:50:F2)
 * - Сильный сигнал (-40 dBm) для избежания угрозы WEAK_SIGNAL
 * - Учтена логика SecurityManager.isSuspiciousMacAddress() (проверка второго бита)
 */
...
class SecurityManagerTest {
    ...
    fun `analyze network security should detect WPA2 as low risk`() {
        // Given
        // ИСПРАВЛЕНО: Используем безопасное имя без подозрительных слов
        // Используем валидный MAC и сильный сигнал
        val wifiInfo = createWifiInfo(
            ssid = "SecureHome",
    ...
    fun `analyze network security should detect WPA3 as low risk`() {
        // Given
        // ИСПРАВЛЕНО: Используем безопасное имя и валидные параметры
        val wifiInfo = createWifiInfo(
            ssid = "SecureOffice",
    ...
    fun `analyze network security should provide appropriate recommendations`() {
        ...
        // Then
        assertTrue("Should have recommendations", result.recommendations.isNotEmpty())
        // ИСПРАВЛЕНО: Проверяем на правильный текст рекомендаций из SecurityManager.generateRecommendations()
        // Для OPEN_NETWORK рекомендация: "Избегайте подключения к открытым сетям без защиты"
        assertTrue("Should recommend avoiding open networks", 
            result.recommendations.any { it.contains("открытым сетям") || it.contains("открытых сетей") })
    }
    ...
    fun `analyze network security should handle multiple threats`() {
        ...
        assertTrue("Should contain suspicious MAC", 
            result.threats.contains(ThreatType.SUSPICIOUS_BSSID))
        // ИСПРАВЛЕНО: Для WEP SecurityManager.analyzeEncryptionType() устанавливает HIGH
        // и НЕ проверяет сигнал (SecurityManager.kt:107-109)
        // WEAK_SIGNAL добавляется только для WPA2 и WPA3 (SecurityManager.kt:115-125)
        // Поэтому не проверяем наличие WEAK_SIGNAL для WEP
    }
    ...
    /**
     * ИСПРАВЛЕНО: Helper метод для создания WifiInfo с безопасными значениями по умолчанию
     * 
     * Ключевые изменения:
     * - bssid по умолчанию: "00:50:F2:11:22:33" (Microsoft OUI, не локально администрируемый)
     * - signalStrength по умолчанию: -40 dBm (сильный сигнал, избегаем WEAK_SIGNAL)
     * - Старый MAC "AA:BB:CC:DD:EE:FF" имел второй бит = 1 (локально администрируемый)
     *   и вызывал срабатывание SecurityManager.isSuspiciousMacAddress()
     */
    private fun createWifiInfo(
        ssid: String = "TestNetwork",
        bssid: String = "00:50:F2:11:22:33", // ИСПРАВЛЕНО: Валидный Microsoft OUI
        encryptionType: EncryptionType = EncryptionType.WPA2,
        signalStrength: Int = -40, // ИСПРАВЛЕНО: Сильный сигнал
