...
interface ThreatDao {
    ...
    /**
     * Добавить несколько угроз атомарно
     * 
     * @Transaction гарантирует, что все угрозы будут добавлены или ни одна
     */
    @Transaction
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    ...
    /**
     * Пакетное разрешение угроз атомарно
     * 
     * @Transaction гарантирует атомарность всех обновлений
     */
    @Transaction
    suspend fun resolveThreats(threatIds: List<Long>, timestamp: Long, note: String? = null) {
        threatIds.forEach { threatId ->
            resolveThreat(threatId, timestamp, note)
        }
    }
    
    /**
     * Пакетная пометка угроз как уведомлённых
     * 
     * @Transaction гарантирует атомарность
     */
    @Transaction
    suspend fun markThreatsAsNotified(threatIds: List<Long>) {
        threatIds.forEach { threatId ->
            markThreatAsNotified(threatId)
        }
    }
