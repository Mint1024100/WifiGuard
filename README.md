...
- `NEARBY_WIFI_DEVICES` — сканирование Wi-Fi без привязки к геолокации (Android 13+)
- `WAKE_LOCK` — для фонового мониторинга
- `RECEIVE_BOOT_COMPLETED` — для возобновления мониторинга после перезагрузки
- `FOREGROUND_SERVICE` — для фоновых служб (Android 9+)
- `FOREGROUND_SERVICE_LOCATION` — для служб с доступом к местоположению (Android 14+)
